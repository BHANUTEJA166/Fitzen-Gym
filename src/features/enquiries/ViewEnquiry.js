...
const ViewEnquiry = () => {
  ...
  const deleteEnq = (eid) => {
    deleteEnquiry(eid)
      .then(() => {
        ...
        setEnquiries(prev => prev.filter(e => e.id !== eid))
      })
      .catch((err) => console.log(err))
  }
  ...
  const submitHandler = (e) => {
    e.preventDefault()
    updateEnquiry(enquiry.id, enquiry)
      .then(() => {
          ...
          {enquiries.map((enq, index) => {
            return (
              <tr key={index}>
                ...
                <td>{enq.message}</td>
                <td>
                  <button onClick={() => getOneRecord(enq.id)} data-bs-target='#Update' data-bs-toggle='modal' className='btn btn-primary me-3'>
